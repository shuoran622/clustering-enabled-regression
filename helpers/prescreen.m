function [Gam0, indepReg_info] = prescreen(FC_norm, X_norm, ind_batch, data_info, thres, output)
...
num_edges = nansum(Gam0,[1 2]);
ratio_edges = num_edges/(d*(d-1)/2);

disp(strcat('Remaining number of edges after screening:', {' '}, string(num_edges), {' ('}, string(100*ratio_edges), {'%)'}));


indepReg_info = struct('R_squared', R_squared, 'F_pvalue', F_pvalue, 'X_test', X_test, 'FC_test_pred_mat', FC_test_pred_mat);


indepReg_prescreen = struct('R_squared', R_squared, 'F_pvalue', F_pvalue);

save_path = strcat(output,'/indepReg_prescreen.mat');
save(save_path, 'indepReg_prescreen', '-v7.3');


fprintf('Independent regression info saved as %s\n', save_path);
